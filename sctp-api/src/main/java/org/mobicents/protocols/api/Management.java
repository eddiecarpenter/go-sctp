 ...
 * </p>
 * <p>
 * Management when {@link #start() started} looks for file <tt>XXX_sctp.xml</tt> containing serialized state of
 * underlying {@link Association} and {@link Server}. Set the directory path by calling {@link #setPersistDir(String)}
 * to direct Management to look at specified directory for underlying serialized file.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * Even if <tt>sctp.persist.dir</tt> system property is not set, Management will look at System set property
 * <tt>user.dir</tt>
 * </p>
 *
 * @author amit bhayani
...
public interface Management
{
	...
	/**
	 * Adding ManagementEventListener. This listener is notified when adding/removing servers and associations
	 *
	 * @param listener
	 */
	...
	/**
	 * Removing ManagementEventListener. This listener is notified when adding/removing servers and associations
	 *
	 * @param listener
	 */
	...
	/**
	 * Adding CongestionListener. This listener is notified when adding/removing servers and associations
	 *
	 * @param listener
	 */
	...
	/**
	 * Removing CongestionListener. This listener is notified when adding/removing servers and associations
	 *
	 * @param listener
	 */
	...
	/**
	 * Start the management. No management operation can be executed unless {@link Management} is started. If {@link
	 * Server} and {@link Association} were defined previously, Management should recreate those {@link Server} and
	 * {@link Association}.
	 *
	 * @throws Exception
	 */
	...
	/**
	 * Stop the management. It should persist the state of {@link Server} and {@link Associtaion}.
	 *
	 * @throws Exception
	 */
	...
	/**
	 * This method stops and removes all registered servers and associations Management should be started Use this
	 * method only for test purposes or after total crashes
	 *
	 * @throws Exception
	 */
	 ...
	 * Add new {@link Server}.
	 *
	 * @param serverName                    name of the Server. Should be unique name
	 * @param hostAddress                   IP address that this server will bind to
	 * @param port                          port that this server will bind to
	 * @param ipChannelType                 IP channel type: SCTP or TCP
	 * @param acceptAnonymousConnections    true: this Server accepts Anonymous connections, false: no
	 * @param maxConcurrentConnectionsCount A count of concurrent connections that can accept a Server. 0 means an
	 *                                      unlimited count.
	 * @param extraHostAddresses            When SCTP multi-homing configuration extra IP addresses can be put here If
	 *                                      multi-homing absence this parameter can be null
	 *
	 * @return new Server instance
	 *
	 * @throws Exception Exception if management not started or server name already taken or some other server already
	 *                   has same ip:port
	 */
	 ...
	 * @param port               port that this server will bind to
	 * @param ipChannelType      IP channel type: SCTP or TCP
	 * @param extraHostAddresses When SCTP multi-homing configuration extra IP addresses can be put here If multi-homing
	 *                           absence this parameter can be null
	 *
	 * @return new Server instance
	 *
	 * @throws Exception Exception if management not started or server name already taken or some other server already
	 ...
	 * @param serverName
	 *
	 * @throws Exception Exception if no Server with the passed name exist or Server is started. Before removing server,
	 ...
	 *
	 * @throws Exception Exception if no Server found for given name or Server already started
	 */
	 ...
	 * Stop the Server.
	 *
	 * @param serverName name of the Server to be stopped
	 *
	 * @throws Exception Exception if no Server found for given name or any of the {@link Association} within Server
	 *                   still started. All the Association's must be stopped before stopping Server
	 */
	 ...
	 *
	 * @param hostAddress
	 * @param hostPort    If hostPort==0 this mean the local port will be any vacant port
	 * @param peerAddress
	 * @param peerPort
	 * @param assocName
	 *
	...
	/**
	 * Remove existing Association. Association should be stopped before removing
	 *
	 * @param assocName
	 *
	 * @throws Exception
	...
	/**
	 * Get connection delay. If the client side {@link Association} dies due to network failure or any other reason, it
	 * should attempt to reconnect after connectDelay interval
	 *
	 * @return
	 */
	...
	/**
	 * For outgoing messages congestion control we need to have 3 thresholds - delays of outgoing messages before it
	 * will be sent to IP channel (3 levels - 1, 2, 3). If a delay time in seconds becomes more then value 0, 1 or 2 of
	 * the array CongControl_DelayThreshold, the Association's congestion level becomes to 1, 2 or 3. Threshold 1.
	 *
	 * @return
	 */
	 ...
	 * For outgoing messages congestion control we need to have 3 thresholds - delays of outgoing messages before it
	 * will be sent to IP channel (3 levels - 1, 2, 3). If a delay time in seconds becomes more then value 0, 1 or 2 of
	 * the array CongControl_DelayThreshold, the Association's congestion level becomes to 1, 2 or 3. Threshold 2.
	 *
	 * @return
	 */
	 ...
	 * For outgoing messages congestion control we need to have 3 thresholds - delays of outgoing messages before it
	 * will be sent to IP channel (3 levels - 1, 2, 3). If a delay time in seconds becomes more then value 0, 1 or 2 of
	 * the array CongControl_DelayThreshold, the Association's congestion level becomes to 1, 2 or 3. Threshold 3.
	 *
	 * @return
	 */
	 ...
	 * For outgoing messages congestion control we need to have 3 thresholds - delays of outgoing messages before it
	 * will be sent to IP channel (3 levels - 1, 2, 3). If a delay time in seconds becomes more then value 0, 1 or 2 of
	 * the array CongControl_DelayThreshold, the Association's congestion level becomes to 1, 2 or 3. Array must have 3
	 * values. Threshold 1.
	 *
	 * @param val
	 *
	...
	/**
	 * For outgoing messages congestion control we need to have 3 thresholds - delays of outgoing messages before it
	 * will be sent to IP channel (3 levels - 1, 2, 3). If a delay time in seconds becomes less then value 0, 1 or 2 of
	 * the array CongControl_BackToNormalDelayThreshold, the Association's congestion level reduces to 0, 1 or 2.
	 * Threshold 1.
	 *
	 * @return
	 */
	 ...
	 * For outgoing messages congestion control we need to have 3 thresholds - delays of outgoing messages before it
	 * will be sent to IP channel (3 levels - 1, 2, 3). If a delay time in seconds becomes less then value 0, 1 or 2 of
	 * the array CongControl_BackToNormalDelayThreshold, the Association's congestion level reduces to 0, 1 or 2. Array
	 * must have 3 values. Threshold 1.
	 *
	 * @param val
	 *
	 * @throws Exception
	 ...
	 * will be sent to IP channel (3 levels - 1, 2, 3). If a delay time in seconds becomes less then value 0, 1 or 2 of
	 * the array CongControl_BackToNormalDelayThreshold, the Association's congestion level reduces to 0, 1 or 2. Array
	 * must have 3 values. Threshold 2.
	 *
	 * @param val
	 *
	 * @throws Exception
	 ...
	 * will be sent to IP channel (3 levels - 1, 2, 3). If a delay time in seconds becomes less then value 0, 1 or 2 of
	 * the array CongControl_BackToNormalDelayThreshold, the Association's congestion level reduces to 0, 1 or 2. Array
	 * must have 3 values. Threshold 3.
	 *
	 * @param val
	 *
	 * @throws Exception
	...
	/**
	 * SCTP option: Enables or disables message fragmentation. If enabled no SCTP message fragmentation will be
	 * performed. Instead if a message being sent exceeds the current PMTU size, the message will NOT be sent and an
	 ...
	 * There are three levels of fragment interleave defined level 0 - Prevents the interleaving of any messages level 1
	 * - Allows interleaving of messages that are from different associations level 2 - Allows complete interleaving of
	 * messages.
	 *
	 * @return
	 */
	...
	/**
	 * SCTP option: The maximum number of streams requested by the local endpoint during association initialization For
	 * an SctpServerChannel this option determines the maximum number of outbound streams accepted sockets will
	...
	/**
	 * SCTP option: The maximum number of streams requested by the local endpoint during association initialization For
	 * an SctpServerChannel this option determines the maximum number of inbound streams accepted sockets will negotiate
	 * with their connecting peer.
	 *
	 * @return
	 */
	...
	/**
	 * SCTP option: Enables or disables a Nagle-like algorithm. The value of this socket option is a Boolean that
	 * represents whether the option is enabled or disabled. SCTP uses an algorithm like The Nagle Algorithm to coalesce
	 * short segments and improve network efficiency.
	 *
	 * @return
	 */
	...
	/**
	 * SCTP option: Linger on close if data is present. The value of this socket option is an Integer that controls the
	 * action taken when unsent data is queued on the socket and a method to close the socket is invoked. If the value
	 * of the socket option is zero or greater, then it represents a timeout value, in seconds, known as the linger
	 * interval. The linger interval is the timeout for the close method to block while the operating system attempts to
	 * transmit the unsent data or it decides that it is unable to transmit the data. If the value of the socket option
	 * is less than zero then the option is disabled. In that case the close method does not wait until unsent data is
	 * transmitted; if possible the operating system will transmit any unsent data before the connection is closed.
	 *
	 * @return
	 */
	...
	/**
	 * Get a sending / receiving buffer size per an association (in bytes). Default value is 8192.
	 *
	 * @return
	 */
	...
	/**
	 * Set a sending / receiving buffer size per an association (in bytes). Default value is 8192.
	 *
	 * @param bufferSize
	 */
	 ...
	 *
	 * @param serverName                    name of the Server which will be modified
	 * @param hostAddress                   IP address that this server will bind to
	 * @param port                          port that this server will bind to
	 * @param ipChannelType                 IP channel type: SCTP or TCP
	 * @param acceptAnonymousConnections    true: this Server accepts Anonymous connections, false: no
	 ...
	 *                                      multi-homing absence this parameter can be null
	 *
	 * @throws Exception Exception if management not started or server started
	 */
	 ...
	 * @param peerPort      the peer port that this association will accept connection from
	 * @param serverName    the Server that this association belongs to
	 * @param assocName     name of Association which will be modified
	 * @param ipChannelType IP channel type: SCTP or TCP
	 *
	 * @return
	 *
